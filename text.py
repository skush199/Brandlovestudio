...
result = app.invoke(
    {
        ...
        """,
        "question_brand": """
        Build {Brand_Name} brand guardrails using ONLY the provided {Brand_Name} assets and retrieved brand context. Do not use external knowledge.

        Rules:
        - Use only information grounded in the provided brand data
        - Prefer explicit values where available
        - If a rule is not explicitly written but can be directly derived from fields such as brand description, audience, tone, emotions, visual identity, color palette, typography, strategy, content behaviour, what to do, what not to do, and compliance restrictions, convert that into a practical brand rule
        - Do NOT invent exact numeric values, exact coordinates, exact logo measurements, or new font names
        ...
        5) Typography hierarchy rules (headline/body) — include font names only if present
